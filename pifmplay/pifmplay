...
#first default the freq, but overwrite if needed (see below)
frequency=91.3
#shuffle defaults to false but is set to true if found in params
shuffle=false

# if there is a second parameter, use it as frequency instead.
if [ $# -ge 2 ]; then

# make sure the second parameter isnt command to shuffle, if it is, use default freq
if [ "$2" = "s" ]; then
  shuffle=true
echo "SHUFFLE HAS BEEN TURNED ON"
else
  frequency=$2
fi
fi

#if there a 3rd param, it may be shuffle being requested
if [ $# -eq 3 ]; then
	if [ "$3" = "s" ]; then
  shuffle=true
echo "SHUFFLE HAS BEEN TURNED ON"
...
# cleaning gpio pin 4 when keyboard interupter is called CONTROL + C
trap control_c INT
...
# shuffles a folders files to play
playfoldershuffle() {
echo "DEBUG: playfoldershuffle"
#this should play all the files in the folder
while :
do

ls $1 |sort -R |tail -$N |while read file; do
playit "$1/$file" $2
...
echo "playing $1"
	...
	echo "DEBUG: playing shuffled as requested"
		playfoldershuffle "$1" $2
	else
		playfolder "$1" $2
		fi
	elif [ -f "$1" ]; then
